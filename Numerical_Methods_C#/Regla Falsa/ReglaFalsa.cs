...
namespace ValorAgregado
{
    class ReglaFalsa
    {
        static void Main(string[] args) {
            ...
            Console.WriteLine("Ingrese el numero maximo de iteraciones: ");
            iteraciones = int.Parse(Console.ReadLine());
            while (iteraciones <= 0) {
                Console.WriteLine("El numero de iteraciones debe ser mayor a 0, ingresela nuevamente: ");
                iteraciones = int.Parse(Console.ReadLine());
            }
        ...
        public static void metodoBiseccion (string func, double xi, double xs, double tolerancia, int iteraciones) {
            double fxi, fxs, fxm, xm, errorAbs, errorRel, xaux;
            int cont;

            Function fx = new Function(func);
